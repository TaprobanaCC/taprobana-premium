...
const clubName =
  process.env.NEXT_PUBLIC_CLUB_NAME || 'Taprobana Sports Club';
...
export default function Home() {
  ...
  async function addAnnouncement() {
    const item = {
      ...
      body: adminForm.body,
      category: adminForm.category || 'Notice'
    };

    if (!item.title || !item.body) {
      return setMessage('Add title and body for the announcement.');
    }
  ...
  async function addEvent() {
    const item = {
      title: adminForm.eventTitle,
      event_date: adminForm.eventDate,
      location: adminForm.eventLocation,
      status: 'Open'
    };

    if (!item.title || !item.event_date) {
      return setMessage('Add event title and date.');
    }
    ...
    return (
      <main className="launch-screen">
        ...
        <section className="hero-card prelaunch-card">
          ...
          <button className="gold-button large" onClick={startLaunch}>
            Start Live Launch
          </button>

          <button className="ghost-button" onClick={() => setStage('live')}>
            Skip to Website
          </button>
        </section>
      </main>
    ...
    <main className="site-shell">
      <nav className="topbar">
        <div className="brand">
          ...
          <div>
            ...
            <small>Premium Member Platform</small>
          </div>
        </div>
      ...
      {sessionWarning && (
        <div className="warning-bar">
          Your secure session is inactive. Move or click to continue.
        </div>
      )}
        ...
        <section className="home-grid">
          <div className="hero-panel">
            <span className="eyebrow">
              Launched for the Sri Lankan sporting community in the UAE
            </span>

            <h1>Taprobana Digital Platform</h1>

            <p>
              A premium digital club experience with member profiles, digital
              cards, news, events, learning hub previews and committee-controlled
              content.
            </p>

            <div className="hero-actions">
              <button className="gold-button" onClick={() => setView('auth')}>
              ...
              >
                Explore Visitor Pavilion
              </button>
            </div>
          </div>

          <div
            className="split-card visitor-card"
            onClick={() => setView('visitor')}
          >
            <h2>Visitor Pavilion</h2>
            <p>About us, achievements, gallery, sponsors and public highlights.</p>
          </div>

          <div
            className="split-card member-card"
            onClick={() => setView(user ? 'dashboard' : 'auth')}
          >
            <h2>Member Pavilion</h2>
            <p>Secure access for founding digital members and club community features.</p>
          </div>
        </section>
      )}
      ...
      {view === 'dashboard' && user && (
        <section className="dashboard">
          <header className="dashboard-hero">
            <div>
              <span className="eyebrow">Member Pavilion</span>
              <h1>Welcome, {profile?.full_name || user.email}</h1>
              <p>Founding Digital Member dashboard for Taprobana Sports Club.</p>
            </div>

            <div className="status-stack">
              <span className="status active">ACTIVE MEMBER</span>
              <span className="status founding">FOUNDING DIGITAL MEMBER</span>
            </div>
          </header>
          ...
          <div className="dashboard-grid">
            <article className="digital-card">
              <div>
                <span>Taprobana Sports Club</span>
              ...
              <QRCodeSVG
                value={`${clubName}:${memberNo}:${user.email}`}
                size={96}
                bgColor="transparent"
                fgColor="#f6d36b"
              />
            ...
            <section className="panel">
              ...
              {events.map((item) => (
                <div className="list-item" key={item.id}>
                  ...
                  <button className="ghost-button small">I am attending</button>
                </div>
              ))}
            </section>
          </div>
          ...
          <section className="cards-grid compact">
            {[
              ...
              <article className="feature-card" key={title}>
                <LoadingBadge>Coming Soon</LoadingBadge>
                <h3>{title}</h3>
                <p>Premium module preview for the Taprobana roadmap.</p>
              </article>
            ))}
          </section>
          ...
          <section className="panel profile-panel">
            ...
            <button className="gold-button" onClick={updateProfile}>
              Save Profile
            </button>
          </section>
        </section>
      ...
      {view === 'admin' && admin && (
        <section className="admin-section">
          ...
          <p>
            Use this launch-ready panel to add visible notices and events. With
            Supabase configured, records are saved to the database.
          </p>
          ...
          <div className="two-column">
            <section className="panel">
              ...
              <input
                ...
                onChange={(e) =>
                  setAdminForm({ ...adminForm, category: e.target.value })
                }
              />
              ...
              <textarea
                ...
                onChange={(e) =>
                  setAdminForm({ ...adminForm, body: e.target.value })
                }
              />

              <button className="gold-button" onClick={addAnnouncement}>
                Publish Notice
              </button>
            </section>
            ...
            <section className="panel">
              ...
              <input
                ...
                value={adminForm.eventTitle}
                onChange={(e) =>
                  setAdminForm({ ...adminForm, eventTitle: e.target.value })
                }
              />

              <input
                type="date"
                value={adminForm.eventDate}
                onChange={(e) =>
                  setAdminForm({ ...adminForm, eventDate: e.target.value })
                }
              />

              <input
                placeholder="Location"
                value={adminForm.eventLocation}
                onChange={(e) =>
                  setAdminForm({ ...adminForm, eventLocation: e.target.value })
                }
              />

              <button className="gold-button" onClick={addEvent}>
                Add Event
              </button>
            </section>
          </div>
