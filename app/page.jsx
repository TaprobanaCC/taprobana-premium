...
const videos = [
  { id: 'rFqM4aJ9Oww', title: 'Division 1 Champions Final Highlights' },
  { id: '3CyoUkxS1hk', title: 'Player Performance Highlights' },
  { id: 'ixEvpQ75x1A', title: 'Division 4 Champions Journey' },
  { id: 'LKdtp5FMLyk', title: 'Taprobana Cricket Club Legacy' },
  { id: 'u7cx0ourLfA', title: 'Super 12 Decider Highlights' },
  { id: '3ZVwarpkiUI', title: 'Division 2 Semi Final Highlights' },
  { id: 'nh1ix-6HNDc', title: 'Division 2 Final Highlights' }
];
...
export default function Home() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && window.location.search.includes('live=1')) setStage('live');
  }, []);
  ...
  async function signUp() {
    ...
    if (!isSupabaseConfigured || !supabase) {
      ...
      setUser(demoUser); setProfile(demoProfile); setView('dashboard');
  ...
  async function login() {
    ...
    setBusy(false);
    if (error) return setMessage(error.message);
    setUser(data.user); setView('dashboard');
  }

  async function signOut() {
    if (isSupabaseConfigured && supabase) await supabase.auth.signOut();
    setUser(null); setProfile(null); setView('home'); setMessage('Signed out successfully.');
  }
  ...
  async function addAnnouncement() {
    ...
    setAdminForm({ ...adminForm, title: '', body: '' }); setMessage('Announcement added.');
  }
  ...
  async function addEvent() {
    ...
    setAdminForm({ ...adminForm, eventTitle: '', eventDate: '', eventLocation: '' }); setMessage('Event added.');
  }
  ...
  return (
    <main className="site-shell">
      ...
      {view === 'visitor' && (
        <section className="content-section">
          ...
          <div className="cards-grid">
            <article className="feature-card"><img src="/images/supun.jpg" alt="About Taprobana" /><h3>About Taprobana</h3><p>Founded in 2013, Taprobana Sports Club has grown into one of the most active Sri Lankan sporting communities in the UAE. Built on friendship, sportsmanship and community values, the club continues to unite members through sport, social events and community engagement.</p></article>
            <article className="feature-card"><h3>Since 2013</h3><p>For more than a decade, Taprobana Sports Club has created opportunities for friendship, sporting excellence, leadership and community development among Sri Lankans living in the United Arab Emirates.</p></article>
            <article className="feature-card"><div className="victory-images"><img src="/images/cricket-legacy.jpg" alt="Taprobana cricket trophy legacy" /><img src="/images/badminton-legacy.jpg" alt="Taprobana badminton trophy legacy" /></div><h3>Our Victories</h3><p>Through cricket, badminton and community events, Taprobana Sports Club has built a proud legacy of championships, achievements and unforgettable moments.</p></article>
            ...
            <article className="feature-card match-archive-card"><h3>Taprobana Match Archive</h3><p>Explore historic Taprobana Sports Club matches, tournament highlights, memorable performances and community cricket moments from our video archive.</p><div className="video-grid">{videos.map((video) => <VideoTile key={video.id} id={video.id} title={video.title} />)}</div></article>
      ...
      {view === 'auth' && (
        <section className="auth-wrap"><div className="auth-card"><h1>{mode === 'login' ? 'Member Login' : 'Create Founding Member Account'}</h1><p>Register during launch and enter the Taprobana Member Pavilion.</p>{mode === 'signup' && <input placeholder="Full name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} />}<input placeholder="Email" type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} />{mode === 'signup' && <input placeholder="Mobile" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} />}{mode === 'signup' && <input placeholder="Profession" value={form.profession} onChange={(e) => setForm({ ...form, profession: e.target.value })} />}{mode === 'signup' && <input placeholder="Company" value={form.company} onChange={(e) => setForm({ ...form, company: e.target.value })} />}<input placeholder="Password" type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} />{mode === 'signup' && <input placeholder="Confirm password" type="password" value={form.confirm} onChange={(e) => setForm({ ...form, confirm: e.target.value })} />}<button className="gold-button full" disabled={busy} onClick={mode === 'login' ? login : signUp}>{busy ? 'Processing...' : mode === 'login' ? 'Login' : 'Create Account'}</button><button className="link-button" onClick={() => setMode(mode === 'login' ? 'signup' : 'login')}>{mode === 'login' ? 'New member? Create account' : 'Already registered? Login'}</button>{!isSupabaseConfigured && <LoadingBadge>Demo mode: add Supabase env variables in Vercel for real accounts</LoadingBadge>}</div></section>
      )}

      {view === 'dashboard' && user && (
        <section className="dashboard">
          <header className="dashboard-hero"><div><span className="eyebrow">Member Pavilion</span><h1>Welcome, {profile?.full_name || user.email}</h1><p>Founding Digital Member dashboard for Taprobana Sports Club.</p></div><div className="status-stack"><span className="status active">ACTIVE MEMBER</span><span className="status founding">FOUNDING DIGITAL MEMBER</span></div></header>
          <div className="dashboard-grid"><article className="digital-card"><div><span>Taprobana Sports Club</span><h2>{profile?.full_name || user.email}</h2><p>{memberNo}</p><small>Status: Active</small></div><QRCodeSVG value={`${clubName}:${memberNo}:${user.email}`} size={96} bgColor="transparent" fgColor="#f6d36b" /></article><article className="metric-card"><strong>03</strong><span>Events Attended</span></article><article className="metric-card"><strong>00</strong><span>CPD Hours</span></article><article className="metric-card"><strong>85%</strong><span>Profile Completion</span></article></div>
          <div className="two-column"><section className="panel"><h2>Club News Centre</h2>{announcements.map((item) => <div className="list-item" key={item.id}><span>{item.category}</span><h3>{item.title}</h3><p>{item.body}</p></div>)}</section><section className="panel"><h2>Upcoming Events</h2>{events.map((item) => <div className="list-item" key={item.id}><span>{item.status || 'Open'}</span><h3>{item.title}</h3><p>{item.event_date} {item.location ? `| ${item.location}` : ''}</p><button className="ghost-button small">I am attending</button></div>)}</section></div>
          <section className="cards-grid compact">{['Digital Gallery','Learning Hub','Live Community','Welfare Hub','Business Directory','Job Board','CPD Certificates','Single Device Security'].map((title) => <article className="feature-card" key={title}><LoadingBadge>Coming Soon</LoadingBadge><h3>{title}</h3><p>Premium module preview for the Taprobana roadmap.</p></article>)}</section>
      ...
      {view === 'admin' && admin && (
        <section className="admin-section"><h1>Committee Admin Panel</h1><p>Use this launch-ready panel to add visible notices and events. With Supabase configured, records are saved to the database.</p><div className="two-column"><section className="panel"><h2>Add Announcement</h2><input placeholder="Title" value={adminForm.title} onChange={(e) => setAdminForm({ ...adminForm, title: e.target.value })} /><input placeholder="Category" value={adminForm.category} onChange={(e) => setAdminForm({ ...adminForm, category: e.target.value })} /><textarea placeholder="Message" value={adminForm.body} onChange={(e) => setAdminForm({ ...adminForm, body: e.target.value })} /><button className="gold-button" onClick={addAnnouncement}>Publish Notice</button></section><section className="panel"><h2>Add Event</h2><input placeholder="Event title" value={adminForm.eventTitle} onChange={(e) => setAdminForm({ ...adminForm, eventTitle: e.target.value })} /><input type="date" value={adminForm.eventDate} onChange={(e) => setAdminForm({ ...adminForm, eventDate: e.target.value })} /><input placeholder="Location" value={adminForm.eventLocation} onChange={(e) => setAdminForm({ ...adminForm, eventLocation: e.target.value })} /><button className="gold-button" onClick={addEvent}>Add Event</button></section></div></section>
      )}
    </main>
  );
}
